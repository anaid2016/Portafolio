...
namespace yii\gii\generators\crudARCA;
...
class Generator extends \yii\gii\Generator
{
    ...
    public $baseFachadaPry ='';
    public $baseEntidadFachada ='';
    public $baseControllerPry='';
    public $baseEntidadFachadaTest ='';
    public $controllerClassTest='';
    public $nsa = 'app\models';
    ...
    {
        return ['entidadcontroller.php','entidadfachada.php'];
    }
    ...
    public function validateModelClass()
    {
        ...
        $pk = $class::primaryKey();
        if (empty($pk)) {
             $this->addError('modelClass', "La tabla asociada con $class debe tener llave(s) primaria(s).");
        }
    }
    ...
    public function generate()
    {
          ...
          $whole_length = strlen($this->controllerClass); 
            $right_length = (strlen(strstr($this->controllerClass, '\\')) - 1); 
            $left_length = ($whole_length - $right_length - 1); 
            $start = (0 - ($right_length + 1)); 
            //$this->nsa='tests\unit\controllers'.substr($this->controllerClass, $start); 
            $this->nsa=substr($this->controllerClass, 0, ($left_length + 1)).'tests\unit\\'.''.substr($this->controllerClass, $start+1); 
        
        
          $this->baseFachadaPry=  ltrim('app\\controllers\controllerspry\\'. 'FachadaPry', '\\');
...
//        
        ...
        $this->baseControllerPry =ltrim('app\\controllers\controllerspry\\'.'ControllerPry', '\\'); //($this->controllerClass.
...
//        ;
        ...
        ;
         ...
         $this->controllerClassTest =ltrim($this->nsa.'Test', '\\'); 
    ...
    private function generateForeignKey($fk,$attribute){
        $pk=false;
        $tpk="";
                            ...
                            if ($column){
                                 ...
                                 if ($column->isPrimaryKey){
                                     ...
                                     if($tableSchema->foreignKeys){
                                        $datos= generateForeignKey($tableSchema->foreignKeys,$attribute);
                                        $pk=$datos[0]; 
                                        $tpk=$datos[1]."";
                                     }
    ...
    public function generateActiveField($attribute)
    {
        ...
        //evalua si el campo pertenece a una llave foranea
        ...
        if ($column->phpType === 'boolean') {
        ...
        } elseif (preg_match('/^(fech|fecha|date|datetime|timestamp)$/i', $column->name) ||$column->type === 'date' ||$column->type === 'datetime' || $column->type === 'timestamp' ) {
            return "\$form->field(\$model, '$attribute')->
           ...
           return "\$form->field(\$model, '$attribute')->widget(\yii\\redactor\widgets\Redactor::className(), [
                'clientOptions' => [
                 ...
                 return "\$form->field(\$model, '$attribute')->dropDownList(\yii\helpers\ArrayHelper::map(". Inflector::camelize($tfk)."::find()->all(),'$attribute','$attribute'),['prompt'=>'Indique el valor para $attribute'])";
        }
        else {
    ...
    public function getColumns()
    {
        ...
        $class = $this->modelClass;
        if (is_subclass_of($class, 'yii\db\ActiveRecord')) {
            return $class::getTableSchema()->columns;
        } 
    }
