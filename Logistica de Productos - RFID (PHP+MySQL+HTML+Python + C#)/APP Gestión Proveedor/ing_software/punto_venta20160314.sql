...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER  VIEW `view_productos` AS select `pv_producto`.`Id` AS `Id`,`pv_producto`.`precio` AS `precio`,`pv_producto`.`codbarras` AS `codbarras`,`pv_producto`.`nombre` AS `nombre`,`pv_categorias`.`nombre` AS `categoria`,`pv_categorias`.`Id` AS `idcategoria`,`pv_producto`.`unidad_id` AS `unidad_id`,`pv_unidades`.`nombre` AS `unidades` from ((`pv_producto` left join `pv_categorias` on((`pv_categorias`.`Id` = `pv_producto`.`categoria_id`))) left join `pv_unidades` on((`pv_unidades`.`Id` = `pv_producto`.`unidad_id`))) ;
...
DECLARE cur CURSOR FOR SELECT codbarras,cantidad,preciolinea,pv_producto.Id,NEW.Id as ordenv FROM pv_producto LEFT JOIN pv_productosoc ON  pv_productosoc .producto_id=pv_producto.Id WHERE pv_productosoc .orden_id=NEW.Id;
...
OPEN cur;
 ...
 ins_loop: LOOP
            ...
            INSERT INTO bodega.com_pedidos2   SET codbarras=ids,cantidad=ids2,preciolinea=ids3,linea_cliente=ids4,pedido_id=@Idpedido,orden_compra=ids5;     
        END LOOP;
...
ELSEIF(NEW.estado_id=3 and OLD.estado_id=2) THEN
