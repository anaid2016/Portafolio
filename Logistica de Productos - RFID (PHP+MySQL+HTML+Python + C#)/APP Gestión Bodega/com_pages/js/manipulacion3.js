$(document).ready(function(){
	...
	$(document).on('click','.clsAgregarFila',function(){
		...
		var strNueva_Fila='<tr id="linea'+real+'">'+
			...
			'<td width="200"><input type="hidden" class="clsAnchoTotal"  name="a'+real+'" id="ca'+real+'" size="5" /><input type="hidden" class="clsAnchoTotal"  name="b'+real+'" id="cb'+real+'" size="5"/><input type="text" readonly="readonly" name="c'+real+'" id="cc'+real+'" size="20"/><input type="button" value="AP" onclick="ventana('+real+')"  style="display:inline" id="btnap'+real+'" /></td>'+
			'<td><input type="text" class="clsAnchoTotal" name="d'+real+'" id="cd'+real+'"></td>'+
			'<td><input type="text" class="clsAnchoTotal" name="e'+real+'" id="ce'+real+'" ></td>'+
			'<td><input type="text" class="clsAnchoTotal" name="f'+real+'" id="cf'+real+'" ></td>'+
			'<td><input type="text" class="clsAnchoTotal" name="g'+real+'" id="cg'+real+'" onblur="OnBlurInput(this,'+real+')" ></td>'+
			'<td><input type="text" class="clsAnchoTotal" name="h'+real+'" id="ch'+real+'" ></td>'+
			'<td><input type="text" class="clsAnchoTotal" name="i'+real+'" id="ci'+real+'" ></td>'+
			'<td><input type="text" class="clsAnchoTotal" name="j'+real+'" id="cj'+real+'" ></td>'+
	...
	$(document).on('click','.clsEliminarFila',function(){
		...
		*/
		...
		var vbaseelim=document.getElementById('ch'+numlinea).value;
		var vgravelim=document.getElementById('cj'+numlinea).value;
		var desaparecidas=document.getElementById('eliminadas').value;
		var newdesaparecida=document.getElementById('cb'+numlinea).value;
		...
		anterior=document.getElementById('cantidad2').value;
		document.getElementById('cantidad2').value=(Number(anterior)-1);
		//alert("revisando 6");
		anterior2=document.getElementById('cantidad').value;
		document.getElementById('cantidad').value=(Number(anterior2)-1);
