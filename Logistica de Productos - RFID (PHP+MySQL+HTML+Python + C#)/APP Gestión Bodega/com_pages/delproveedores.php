...
<!--Menu de Solapas -->
	<div id="tabsI">
    	 <ul>
            ...
            <li><a href="<?php echo (!empty($_GET['Id']))? 'dirproveedores.php?Id='.$_GET['Id'].'&view=true':'' ?>" title="Direcciones"><span>Direcciones</span></a></li>
         </ul>
    </div>
...
<div id="contenidof">
    ...
    <?php
            ...
            if(!empty($_POST['Guardar']) && empty($_GET['Id'])){
				...
				$delete=mysql_query("DELETE FROM com_direccionproveedor WHERE proveedor_id='$id' ");
				if(!$delete)
				$error=1;
					
				$delete=mysql_query("DELETE FROM com_proveedores WHERE Id='$id' ");
				if(!$delete)
				$error=1;
				...
				if($error) {
					...
					mysql_query("COMMIT");
					echo "Proveedor Borrado con éxito";
				}
