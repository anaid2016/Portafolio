<?php
	...
	if(!empty($_GET['Id'])){
				...
				$id=$_GET['Id'];
				$buscar=mysql_query("SELECT * FROM com_direccionproveedor WHERE proveedor_id='$id'",$conexion);
				while($resultado=mysql_fetch_assoc($buscar)){
					array_push($arrid,$resultado['Id']);	
					array_push($arrprov,$resultado['proveedor_id']);
...
<?php
	if(empty($_GET['view'])){
		$enlace="agproveedor.php";
	}else{
		$enlace="delproveedores.php";
	}
?>
	<!--Menu de Solapas -->
	<div id="tabsI">
...
<div id="contenidof">
	...
	//Guardando Direcciones de Proveedores
            ...
            if(!empty($_POST['Guardar']) && !empty($_POST['Id'])){
				...
				if(!empty($_POST['Idborrarn'])){
				$vborrar=explode(",",$_POST['Idborrarn']);
				for($t=0;$t<count($vborrar);$t++){
						if($vborrar[$t]!=''){
							$insertar=mysql_query("DELETE FROM com_direccionproveedor WHERE Id='$vborrar[$t]' ");
							if(!$insertar)
								$error=1;
						}
					}
				...
				for($b=1;$b<=$cantidad;$b++){
					if($vector6[$b]==''){
						$insertar=mysql_query("INSERT INTO com_direccionproveedor SET proveedor_id='$id',direccion='$vector2[$b]',telefono='$vector3[$b]',ciudad_id='$vector4[$b]',sede='$vector1[$b]'");
						if(!$insertar)
							$error=1;
					}
				}
				...
				if($error==1) {
					...
					echo "Esta insertando una direccion ya existente o Esta eliminando una direccion asociada a un proceso";
				} else {
					mysql_query("COMMIT");
					echo "Direcciones Agregadas con éxito";
				}
    ...
    <div id="divContenedorTabla">
		...
		<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
			<table align="center" width="700">
				...
				?>
                	<tr id="linea1">
                    	<input type="hidden" name="idp1" value="" id="idpr1">
				...
				?>
                		<tr id="linea<?php echo $d; ?>">
                        	<input type="hidden" name="idp<?php echo $d; ?>" value="<?php echo (isset($arrid[$c]))? $arrid[$c]:'' ?>" id="idpr<?php echo $d; ?>" />
				...
				<tfoot>
					<tr>
						<td colspan="4" align="right">
                            ...
                            <input type="hidden" name="Idborrarn" id="Idborrar"/>
                            <input type="submit" value="Registrar Direcciones" name="Guardar" />
							<?php
							}
							?>
